...
const PostOptionsMenu: React.FC<PostOptionsMenuProps> = ({ post, currentUser, onEdit, onDelete, onReport, onBlock }) => {
    ...
    // FIX: Add a guard to ensure post.author is a User object before accessing its properties.
    const postAuthor = typeof post.author === 'object' ? post.author : null;
    if (!postAuthor) return null;

    const isOwnPost = postAuthor.id === currentUser.id;

    const handleBlockClick = () => {
        setIsOpen(false);
        const confirmBlock = window.confirm(`Are you sure you want to block @${postAuthor.username}? You will no longer see their posts or comments.`);
                            ...
                            <>
                             ...
                             <>
                                ...
                                {onBlock && <button onClick={handleBlockClick} className="w-full text-left flex items-center gap-3 px-4 py-2 hover:bg-[var(--hover-bg)] text-sm text-red-500">
                                    {/* FIX: Use guarded postAuthor object */}
                                    <BlockUserIcon className="w-4 h-4" /> Block @{postAuthor.username}
                                </button>}
                            </>
                        )}
                    </motion.div>
...
const PostCard: React.FC<PostCardProps> = ({ post, currentUser, allUsers, onViewProfile, onLikePost, onLikeComment, onCommentPost, onRatePost, onEditPost, onDeleteRequest, onReportRequest, onBlockUser, onRepost, isCommentingAllowed = true, pinConfig }) => {
    ...
    // FIX: Add guards to ensure author properties can be safely accessed.
    const postAuthor = typeof post.author === 'object' ? post.author : null;
    const originalPostAuthor = typeof originalPost.author === 'object' ? originalPost.author : null;
    const showOptionsMenu = onEditPost || onDeleteRequest || onReportRequest || onBlockUser;

    // FIX: Don't render card if the main author is not a valid User object.
    if (!originalPostAuthor) {
        return <GlassCard><p>Error: Post author not found.</p></GlassCard>;
    }

    return (
        <GlassCard className="w-full">
            {post.repostOf && postAuthor && (
                <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)] mb-3">
                    <RepostIcon className="w-4 h-4"/>
                    {/* FIX: Use guarded postAuthor object */}
                    <button onClick={() => onViewProfile(postAuthor.id)} className="font-bold hover:underline">{postAuthor.name}</button> reposted
                </div>
            )}
            <div className={`flex items-start space-x-3 sm:space-x-4 ${post.repostOf ? 'pl-4 border-l-2 border-[var(--card-border)]' : ''}`}>
                {/* FIX: Use guarded originalPostAuthor object */}
                <button onClick={() => onViewProfile(originalPostAuthor.id)} className="flex-shrink-0 transition-transform duration-200 hover:scale-105">
                    <img src={originalPostAuthor.avatarUrl} alt={originalPostAuthor.name} className="w-12 h-12 rounded-full" />
                </button>
                <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between">
                    <div>
                        {/* FIX: Use guarded originalPostAuthor object */}
                        <button onClick={() => onViewProfile(originalPostAuthor.id)} className="text-left">
                            <p className="font-bold text-[var(--text-primary)] hover:underline">{originalPostAuthor.name}</p>
                        </button>
                        {/* FIX: Use guarded originalPostAuthor object */}
                        <p className="text-sm text-[var(--text-secondary)]">@{originalPostAuthor.username}</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <p className="text-xs text-[var(--text-secondary)] whitespace-nowrap">{new Date(originalPost.timestamp).toLocaleDateString()}</p>
                        {showOptionsMenu && <PostOptionsMenu 
                            ...
                            // FIX: Use guarded originalPostAuthor object
                            onBlock={onBlockUser ? () => onBlockUser(originalPostAuthor.id) : undefined}
                        />}
                    </div>
                </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-[var(--card-border)] transition-all duration-300">
                            ...
                            <div className="space-y-3">
                                {sortedComments.length > 0 ? sortedComments.map(comment => {
                                    ...
                                    // FIX: Guard against comment.author being a string
                                        ...
                                        <div key={comment.id} className="flex items-start gap-3">
                                            ...
                                            <div className="bg-[var(--hover-bg)] p-3 rounded-lg flex-1">
                                                <div className="flex items-baseline gap-2">
                                                    <p className="font-semibold text-sm text-[var(--text-primary)]">{commentAuthor.name}</p>
                                                    <p className="text-xs text-[var(--text-secondary)]">{new Date(comment.timestamp).toLocaleDateString()}</p>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-4 mt-2 text-xs text-[var(--text-secondary)]">
                                                    ...
                                                    <button 
                                                        // FIX: Pass the guarded commentAuthor object
                                                        onClick={() => handleReply(commentAuthor)}
