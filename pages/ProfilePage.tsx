...
const ProfilePage: React.FC<ProfilePageProps> = ({ 
    ...
    const [followModalData, setFollowModalData] = useState<{ title: string, userIds: string[] }>({ title: '', userIds: [] });
    ...
    const { userPosts, likedPosts, commentedPosts } = useMemo(() => {
        // FIX: Check author type is object before accessing id property.
        const userPosts = allPosts.filter(p => typeof p.author === 'object' && p.author.id === profileUser.id);
        const likedPosts = allPosts.filter(p => p.likedBy.includes(profileUser.id));
        const commentedPostsSet = new Set<Post>();
        allPosts.forEach(p => {
            // FIX: Check author type is object before accessing id property.
            if (p.comments.some(c => typeof c.author === 'object' && c.author.id === profileUser.id)) {
    ...
    const displayedPosts = useMemo(() => {
        ...
        const sorted = [...filteredByRating];
        if (sortBy === 'top') {
            sorted.sort((a, b) => b.likes - a.likes);
        } else {
            // Assuming initial 'allPosts' is sorted by timestamp descending from query
            // The default order should be fine for 'recent'
        }
        return sorted;
